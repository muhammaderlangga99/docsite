...
@section('title', 'Host2Host CDCP & QRPS')

@section('dashboard-content')
<div class="flex items-center justify-between mb-6">
    <div>
        <p class="text-xs uppercase tracking-[0.08em] text-slate-500 mb-1">Host2Host</p>
        <h1 class="text-2xl font-semibold text-slate-900">CDCP &amp; QRPS</h1>
...
<div class="border border-slate-200 bg-white text-slate-900 rounded-xl p-6 text-sm shadow-sm space-y-4">
    ...
    <div class="grid md:grid-cols-2 gap-3">
        ...
        <div class="border border-slate-200 rounded-lg p-3 bg-slate-50/70">
            <p class="text-xs uppercase text-slate-500 tracking-[0.08em] mb-1">TID CDCP</p>
            <div class="flex items-center justify-between gap-2">
                @if($cdcpTid)
                    <span class="font-semibold text-slate-900 break-all">{{ $cdcpTid }}</span>
                @else
                    <span class="text-orange-600 font-light text-xs">TID belum tersedia. Silakan generate CDCP terlebih dahulu.</span>
                @endif
                <button type="button" class="text-xs px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 copy-btn" data-copy="{{ $cdcpTid }}">Copy</button>
            </div>
        </div>
        <div class="border border-slate-200 rounded-lg p-3 bg-slate-50/70">
            <p class="text-xs uppercase text-slate-500 tracking-[0.08em] mb-1">TID QRIS</p>
            <div class="flex items-center justify-between gap-2">
                @if($qrisTid)
                    <span class="font-semibold text-slate-900 break-all">{{ $qrisTid }}</span>
                @else
                    <span class="text-orange-600 font-light text-xs">TID belum tersedia. Silakan generate QRIS terlebih dahulu.</span>
                @endif
                <button type="button" class="text-xs px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 copy-btn" data-copy="{{ $qrisTid }}">Copy</button>
            </div>
        </div>
    </div>
</div>
